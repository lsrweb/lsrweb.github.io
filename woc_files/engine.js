...
dwr.engine._origScriptSessionId = "D54FE946D75B507AA8B9F2CB85F55489";
...
dwr.engine._defaultPath = "/xgxt/dwr";
...
/**跨域扩展
如果有跨域问�?在dwr加载之前定义
var myDwrPath="your dwr path"
例如：var myDwrPath="/xg/xgxt/dwr"
支持匹配多个使用逗号分隔
[982] 张昌�?***************/
dwr.engine._myDwrPath ="";
dwr.engine._extendDomain=function(path){
	if(myDwrPath){
		dwr.engine._myDwrPath=myDwrPath;
	}
  if(dwr.engine._myDwrPath){
	var domainPath=myDwrPath.split(",");
	for(var i=0;i<domainPath.length;i++){
		myDwrPath=domainPath[i];
		alert(myDwrPath);
		alert("--"+dwr.engine._defaultPath);
		alert(dwr.engine._defaultPath.indexOf(myDwrPath)<0);
		if(dwr.engine._defaultPath.indexOf(myDwrPath)<0){
			dwr.engine._defaultPath=dwr.engine._defaultPath.replace(path,myDwrPath);
			break;
		}
	}
	alert(dwr.engine._defaultPath);
...
dwr.engine._execute = function(path, scriptName, methodName, vararg_params) {
  path=dwr.engine._extendDomain(path);
